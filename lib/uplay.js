...
// confirms it actually works before scripts/uplay-login.js tells the user
// to copy anything into .env.
export async function verifyUplaySession(loginPayload) {
  ...
  if (loginPayload.rememberMeTicket) {
    ...
    // now so it's that (still-valid) exchange we confirm, not just the paste.
    return refreshSession();
  }

  // No remember-me ticket was issued for this login, so there's nothing to
  // exchange - just confirm the pasted ticket itself is accepted by the API.
